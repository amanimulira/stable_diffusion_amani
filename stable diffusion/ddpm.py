...
class DDPMSampler:
    ...
    def __init__(self, generator: torch.Generator, num_training_steps=1000, beta_start: float = 0.00085, beta_end: float = 0.0120):
        # beta represents the variance schedule that controls the amount of noise added at each timestep t during the forward diffusion process.
    ...
    def _get_variance(self, timestep: int) -> torch.Tensor:
        ...
        vairance = (1 - alpha_prod_t_prev) / (1 - alpha_prod_t) * current_beta_t

        vairance = torch.clamp(vairance, min=1e-20)
